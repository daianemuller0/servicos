...
As três guias da planilha viraram **uma única tela** (`/servicos/proposta`), e as
macros VBA (`MóduloProposta`, `MóduloCusto`, `MóduloWord`, `ClasseCusto`…) viraram
código C#. A página é dividida em seções numeradas, na ordem de preenchimento:
1 · dados do cliente e da proposta, 2 · margem/riscos/impostos, 3–4 · custos de
mão de obra e despesas, 5 · composição do preço e 6 · os itens já precificados,
com o resumo e os botões (cadastrar, PDF, Word, e-mails) fixos na lateral.
...
│       ├── Login.razor
│       ├── GerarProposta.razor    # a tela única: cadastro + margem + custos + proposta
...
A proposta em andamento vive em `Data/Rascunho.cs`, registrado como *scoped* —
no Blazor Server isso equivale ao circuito do usuário, então navegar pelo menu
preserva tudo sem gravar nada no banco.

Como um F5 começa um circuito novo, `Components/PaginaProposta.cs` também grava o rascunho em JSON no
